...
- **可一键彻底关闭**：点播放器上的 **×** 即可关闭——播放器**完全从页面消失**（不留任何按钮，音乐继续播放）；重新打开的方式：对 agent 说「**打开播放器**」（`music` 工具 show 动作），或到 **设置 → 插件 → 插件配置** 的「音乐播放器」卡片点「显示播放器」——打开后默认是小唱片圈吸附在边上（首次使用才完整展示）
...
- **关闭播放器**：展开面板点右上角 **×**（或小圈悬停点 **×**），播放器**完全隐藏**（音乐继续播，页面不留痕迹）；对 agent 说「打开播放器」或到设置 → 插件 → 插件配置点「显示播放器」即可重新打开（默认小圈形态贴边）
